...
pcaps_bccc = [p.strip() for p in run_remote('find /dados/BCCC-cPacket-Cloud-DDoS-2024/extracted/ -type f -name \'*.pcap*\'').splitlines() if p.strip()]
all_pcaps = pcaps_bccc

out_csv = '/tmp/parity_results_bccc.csv'
processed = set()
...
for pcap in all_pcaps:
    ...
    time.sleep(2)
    try: run_remote(f'python3 /home/leonardo.herkenhoff/push_pcap.py {pcap}')
    except Exception as e: print(f"TRex error on {pcap}: {e}")
    time.sleep(1)
    ...
    # just writing a dummy for now so it's ready to be populated
    time.sleep(2)
    try: run_remote(f'python3 /home/leonardo.herkenhoff/push_pcap.py {pcap}')
    except Exception as e: print(f"TRex error on {pcap}: {e}")
    time.sleep(1)
